...
class SQLiteArchive(DBUtility):
    def __init__(self):
        ...
        def setav() -> bool:
            ...
            if self.args.autovacuum and self.args.autovacuum in (
                ...
                avstate2 = self.execquerynocommit("PRAGMA auto_vacuum;",
                                                  one=True,
                                                  returndata=True)
                if not self.args.mode == "compact" and avstate2 == 1:
                    return True
                else:
                    if avstate2 != 1 and avstate != 1 and self.args.verbose or self.args.debug:
                        print(notchanged)
                    return False
                if self.args.verbose or self.args.debug:
                    print("full auto_vacuum")
            elif self.args.autovacuum and self.args.autovacuum in (
                    "incremental", 2, "2") and not avstate == 2:
                self.execquerynocommit("PRAGMA auto_vacuum = 2;")
                avstate2 = self.execquerynocommit("PRAGMA auto_vacuum;",
                                                  one=True,
                                                  returndata=True)
                if not self.args.mode == "compact" and avstate2 == 2:
                    return True
                else:
                    if avstate2 != 2 and avstate != 2 and self.args.verbose or self.args.debug:
                        print(notchanged)
                    return False
                if self.args.verbose or self.args.debug:
                ...
                avstate2 = self.execquerynocommit("PRAGMA_auto_vacuum;",
                                                  one=True,
                                                  returndata=True)
                if not self.args.mode == "compact" and avstate2 == 0:
                    return True
                else:
                    if avstate2 != 0 and avstate != 0 and self.args.verbose or self.args.debug:
                        print(notchanged)
                    return False
                if self.args.verbose or self.args.debug:
        ...
        needsvacuum = False
        if addorcreate and "autovacuum" in self.args and self.args.autovacuum:
            needsvacuum = setav()
        ...
        if addorcreate and "wal" in self.args and self.args.wal and journal_mode not in wal:
            needsvacuum = setwal()
        elif addorcreate and "rollback" in self.args and self.args.rollback and journal_mode not in rollback:
            needsvacuum = setdel()
